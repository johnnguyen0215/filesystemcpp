...
class FileSystem53 {
	char** ldisk; // local disk
	char* buffer; // intermediary for block read/write
	char** OFT; // open file table

	int DESC_BLOCK_NO;
	int MAX_ENTRY_NO; // Max number of bytes per entry, 10 bytes for the file name, 1 byte for the rawDescriptorIndex.
	int MAX_ENTRY_BYTES_PER_BLOCK;

	int CURR_NO_FILES;

	int TOTAL_DIRECTORY_BLOCKS_USED; // This number represents the total number of blocks the directory is currently using
	int TOTAL_DIRECTORY_BYTES; // This number represents how many bytes in total have bee

	int L;  //Number of logical blocks
	int B;  //Block length
	...
	int OFT_SIZE; // 64 bytes for 1 disk block; 1 for position; 1 for file descriptor index
...
public:
	 ...
	 */
	FileSystem53(int DESC_BLOCK_NO, int MAX_ENTRY_NO, int MAX_ENTRY_BYTES_PER_BLOCK,
	 ...
	 */
	int search_dir(std::string symbolic_file_name);
	...
	int find_unfilled_dir_block(); // Sets CURR_DIRECTORY_BLOCK to an index in ldisk[1] that has space for a filename + index (11 Bytes)

	bool check_full_block(char* block);

	bool check_equal_block(int start, int stop, char* b1, char* b2);

	int check_OFT_cp(int index);
